...
import { User, LogOut, Settings, HelpCircle, Heart, FileText, Edit2, MapPin, GraduationCap } from "lucide-react";
...
export default function ProfilePage() {
    ...
    const [stats, setStats] = useState({ notices: 0, favorites: 0 });
    const loading = status === "loading";

    useEffect(() => {
        async function loadStats() {
            if (session?.user) {
                const data = await getUserStats((session.user as any).id);
                setStats(data);
            }
        }
