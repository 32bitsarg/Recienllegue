...
import { Bus, Map as MapIcon, Clock, ArrowRightLeft, Info, ChevronDown, ChevronUp } from "lucide-react";
import styles from "@/app/transporte/Transporte.module.css";
...
export interface TransporteInitialData {
    urbanLines: any[];
    terminalRoutes: any[];
}

interface MobileTransporteProps {
    initialData?: TransporteInitialData;
}

export default function MobileTransporte({ initialData }: MobileTransporteProps) {
    const [activeTab, setActiveTab] = useState("urbano");
    const [urbanLines] = useState<any[]>(initialData?.urbanLines ?? []);
    const [terminalRoutes] = useState<any[]>(initialData?.terminalRoutes ?? []);
